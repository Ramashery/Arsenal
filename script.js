...
/**
 * Безопасно возвращает массив. Если на входе не массив, 
 * вернёт пустой массив и выведет предупреждение в консоль.
 * @param {any} potentialArray - Данные, которые должны быть массивом.
 * @returns {Array} - Исходный или пустой массив.
 */
...
async function initializeSite() {
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            // Данные получены, рендерим страницу
            renderPage();

            // И только после этого скрываем экран загрузки
            loaderOverlay.style.opacity = '0';
            setTimeout(() => {
                loaderOverlay.style.display = 'none';
            }, 500); // Даем время для анимации затухания

        } else {
            // Документа в Firebase нет - это критическая ошибка
            throw new Error("Документ 'main' в Firebase не найден.");
        }
    } catch (error) {
        // Любая ошибка (сеть, права доступа, и т.д.) приведет сюда
        console.error("Критическая ошибка при загрузке данных из Firebase:", error);
        
        // Больше НЕ используем defaultSiteData. Вместо этого показываем ошибку.
        loaderSpinner.style.display = 'none';
        loaderText.style.display = 'none';
        loaderError.style.display = 'block';
        
        // НЕ вызываем renderPage(), чтобы страница осталась пустой под экраном ошибки
    }
