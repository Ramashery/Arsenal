...
async function initializeSite() {
    try {
        // --- НОВАЯ ЛОГИКА ЗАГРУЗКИ ИЗ FIREBASE ---
        ...
        // Ссылка на наш единственный документ с контентом
        ...
        if (docSnap.exists()) {
            ...
            // Если в Firebase еще нет данных, используем данные по умолчанию
        ...
        // Если произошла ошибка (например, нет интернета), используем данные по умолчанию
        siteData = deepCopy(defaultSiteData);
    }

    // После того как данные загружены (из Firebase или по умолчанию), отрисовываем страницу
    renderPage();

    // Весь остальной код инициализации остается без изменений
