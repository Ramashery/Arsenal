...
function renderPage() {
    ...
    // --- ИЗМЕНЕНИЕ: Рендеринг слайдера из новой структуры данных ---
    const sliderWrapper = document.getElementById('trees-slider-wrapper');
    sliderWrapper.innerHTML = globalData.slider.map((globalSlide, index) => {
        const langSlide = data.slider[index] || { title: '', desc: '' }; // Защита от ошибок
        return `
    ...
    // --- ИЗМЕНЕНИЕ: Рендеринг команды из новой структуры данных ---
    const teamWrapper = document.getElementById('team-grid-wrapper');
    teamWrapper.innerHTML = globalData.team.members.map((globalMember, index) => {
        const langMember = data.team.members[index] || { name: '', role: '' }; // Защита от ошибок
        return `
...
async function initializeSite() {
    try {
        ...
        if (docSnap.exists()) {
            ...
            console.warn('Документ в Firebase не найден. Используются данные по умолчанию.');
