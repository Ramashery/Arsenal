...
function renderPage() {
    ...
    // --- ИЗМЕНЕНИЕ: Логика рендеринга для секций с новой структурой ---
    ...
    helpCardsWrapper.innerHTML = getArraySafely(globalData.helpCards).map(globalCard => {
        ...
        return `
            <div class="card fade-in">
                ...
                <p>${langCard.desc}</p>
                ${langCard.details ? `<p id="help-card-details-${globalCard.id}" style="white-space: pre-wrap; overflow-wrap: break-word;">${langCard.details}</p>` : ''}
