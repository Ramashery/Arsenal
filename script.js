...
function renderPage() {
    ...
    // ИЗМЕНЕНИЕ: Логика рендеринга слайдера
    sliderWrapper.innerHTML = globalData.sliderItems.map((globalSlide, index) => {
    ...
    // ИЗМЕНЕНИЕ: Логика рендеринга команды
    teamWrapper.innerHTML = globalData.teamMembers.map(globalMember => {
...
async function initializeSite() {
    try {
        const { getFirestore, doc, getDoc } = await import("https://www.gstatic.com/firebasejs/12.2.1/firebase-app.js");
