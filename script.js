...
function renderPage() {
    ...
    navMenu.innerHTML = navOrder.map(key => {
        const value = data.nav?.[key] || key.charAt(0).toUpperCase() + key.slice(1);
    ...
    // ===== НОВЫЙ КОД: НАПОЛНЕНИЕ ФУТЕРА =====
    document.getElementById('footer-nav-menu').innerHTML = navMenu.innerHTML;
    document.getElementById('footer-social-links-wrapper').innerHTML = socialWrapper.innerHTML;
}
...
async function initializeSite() {
    ...
    // ===== ИЗМЕНЕННЫЙ КОД: ОБРАБОТЧИК СКРОЛЛА ДЛЯ ХЕДЕРА =====
