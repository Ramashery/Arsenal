...
function saveAdminChanges() {
    ...
    setTimeout(() => {
        try {
            ...
            isDirty = false;
            adminNotifications.show('Файл siteData.json сгенерирован! Загрузите его на хостинг.', 'success');
...
function addAdminEventListeners() {
    ...
    document.querySelector('#admin-content-wrapper').addEventListener('click', (e) => {
        ...
        if (actionBtn) {
            ...
            // --- НАЧАЛО ИЗМЕНЕНИЯ: Универсальный обработчик для модальных окон ---
            // Эта функция временно убирает z-index у админ-панели, чтобы `confirm` был кликабельным.
            const confirmAction = (message, callback) => {
                const adminPanel = document.getElementById('admin-panel');
                const originalZIndex = adminPanel.style.zIndex;
                try {
                    adminPanel.style.zIndex = 'auto'; // Временно убираем z-index
                    if (confirm(message)) {
                        callback();
                    }
                } finally {
                    adminPanel.style.zIndex = originalZIndex || '9999'; // Возвращаем z-index
                }
            };
            // --- КОНЕЦ ИЗМЕНЕНИЯ ---
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('admin-close-btn').addEventListener('click', () => {
        let proceed = !isDirty;
        if (isDirty) {
            // --- НАЧАЛО ИЗМЕНЕНИЯ: Исправление бага с модальным окном при закрытии ---
            const adminPanel = document.getElementById('admin-panel');
            const originalZIndex = adminPanel.style.zIndex;
            try {
                adminPanel.style.zIndex = 'auto'; // Временно убираем z-index
