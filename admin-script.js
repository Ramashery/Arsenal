...
const defaultSiteData = {
    ...
    ge: {
        ...
        about: {
            ...
            items: [
                { title: "ადგილი", text: "არსენალის მთა — 70 ჰექტარი ბუნება თბილისის გულში. 1990-იან წლებში, რთულ დროს, აქ ბევრი ხე მოიჭრა და მთამ დაკარგა თავისი სიმწვანის ნაწილი. მას შემდეგ ის რჩება ველურ სივრცედ — სასეირნო, სიჩუმისა და ბუნებასთან შეხვედრის ადგილი.\n\nდღეს არსენალის მთა ინახავს დაკარგული ხეების ხსოვნას და ამავდროულად ხსნის ახალ მომავალს. აქ შეიძლება გაჩნდეს ბაღი — ცоცხალი სივრცე ადამიანებისა და ქალაქისთვის. ყოველი ახალი ხე მთას ძალას უბრუნებს, ჩვენ კი — მისი სილამაზის ხელახლა შეგრძნების შესაძლებლობას." },
...
// --- НОВАЯ УТИЛИТА: Безопасное получение родителя и ключа по пути ---
/**
 * Безопасно получает родительский объект и финальный ключ по строковому пути.
 * @param {object} obj - Корневой объект для поиска.
 * @param {string} path - Строковый путь (например, 'about.items').
 * @param {boolean} createIfMissing - Если true, создаст недостающие объекты на пути.
 * @returns {{parent: object|undefined, key: string|undefined}} - Объект с родителем и ключом.
 */
function getParentAndKeyByPath(obj, path, createIfMissing = false) {
        ...
        case 'slider':
            ...
            html += (data.slider || []).map((slide, index) => `
                <div class="admin-card" data-index="${index}">
                    ...
                    <div class="admin-card-body">
                     ${createImageInput('URL Картинки (синхронизируется)', `slider.${index}.img`, slide.img)}
        ...
        case 'team':
            ...
            html += (data.team.members || []).map((member, index) => `
                <div class="admin-card" data-index="${index}">
                    ...
                    <div class="admin-card-body">
                        ${createImageInput('URL Фото (синхронизируется)', `team.members.${index}.img`, member.img)}
...
async function saveAdminChanges() {
    ...
    try {
        ...
        const oldCategoryIds = siteData.global.treeCategories.map(c => c.id);
        const newCategoryIds = [];
        
        // Список путей, где URL изображений должны синхронизироваться
        const syncedMediaPaths = ['slider', 'team.members'];
        ...
        document.querySelectorAll('#admin-content-wrapper input, #admin-content-wrapper textarea').forEach(input => {
            ...
            // Сохраняем новые ID категорий для последующей миграции текстовых полей
            if (keys.includes('treeCategories') && keys[keys.length -1] === 'id') {
                 newCategoryIds[keys[2]] = input.value; // keys[2] - это индекс
            }

            const sectionKey = keys[0];
            const lastKey = keys[keys.length - 1];
            
            // Проверяем, является ли это поле синхронизируемым медиа-полем
            if (syncedMediaPaths.includes(sectionKey) && lastKey === 'img' && keys.length > 2) {
                // Это поле с изображением в слайдере или команде. Синхронизируем его.
                const index = keys[1]; // Индекс элемента в массиве
                
                ['ru', 'en', 'ge'].forEach(lang => {
                    if (tempSiteData[lang]?.[sectionKey]?.[index]) {
                        tempSiteData[lang][sectionKey][index][lastKey] = input.value;
                    }
                });
            } else {
                // Стандартная логика для всех остальных полей
                let current;
                if (keys[0] === 'global' || currentAdminSection === 'trees') {
        ...
        // Миграция текстовых данных при смене ID категории
...
// --- ПЕРЕПИСАННАЯ ЛОГИКА СИНХРОНИЗАЦИИ ---
/**
 * Синхронизирует длину массивов в разных языковых версиях,
 * безопасно создавая недостающие структуры данных.
 */
function synchronizeDataArrays() {
    const pathsToSync = [
        'about.items', 'slider', 'events.items', 'help.cards', 'materials.items', 'team.members'
    ];
    ...
    pathsToSync.forEach(path => {
        ...
        // 1. Собрать все массивы и найти максимальную длину
        languages.forEach(lang => {
            const { parent, key } = getParentAndKeyByPath(siteData[lang], path, true); // true - создаем путь
            if (parent && typeof parent[key] === 'undefined') {
                parent[key] = []; // Если массива нет, создаем его
            }
            const arr = parent[key];
            if (Array.isArray(arr)) {
                arraysByLang[lang] = arr;
        ...
        // 2. Дополнить короткие массивы до максимальной длины
        languages.forEach(lang => {
            const arr = arraysByLang[lang];
            if (arr && arr.length < maxLength) {
                let newItemTemplate = {};
                if(path === 'about.items') newItemTemplate = { title: "Новый заголовок", text: "Новый текст." };
                if(path === 'slider') newItemTemplate = { img: "https://source.unsplash.com/1200x800/?placeholder", title: "Новый слайд", desc: "Новое описание." };
                if(path === 'team.members') newItemTemplate = { img: "https://source.unsplash.com/300x300/?person", name: "Новый сотрудник", role: "Должность" };
                if(path === 'events.items') newItemTemplate = { title: "Новое событие", desc: "Новое описание." };
                if(path === 'materials.items') newItemTemplate = { title: "Новый материал", points: ["Пункт 1"] };
                ...
                for (let i = 0; i < diff; i++) {
                    arr.push(deepCopy(newItemTemplate));
                }
                console.log(`Synced path "${path}" for lang "${lang}". Added ${diff} items.`);
                wasModified = true;
            }
        });
    });
    ...
    if (wasModified) {
        isDirty = true;
        adminNotifications.show('Структура данных была синхронизирована. Пожалуйста, сохраните изменения.', 'warning', 6000);
    }
}

// --- ПЕРЕПИСАННАЯ ЛОГИКА ОБРАБОТКИ СОБЫТИЙ ---
function addAdminEventListeners() {
    ...
    document.querySelector('#admin-content-wrapper').addEventListener('click', (e) => {
        ...
        if (actionBtn) {
            ...
            if (path === 'treeCategories') { // Логика для глобальных категорий деревьев (остается без изменений)
                if (action === 'add') {
                ...
                // Новая, надежная логика для синхронизированных массивов
                const languages = ['ru', 'en', 'ge'];

                if (action === 'add') {
                    let newItemTemplate = {};
                    if(path === 'about.items') newItemTemplate = { title: "Новый заголовок", text: "Новый текст." };
                    if(path === 'slider') newItemTemplate = { img: "https://source.unsplash.com/1200x800/?nature", title: "Новый слайд", desc: "Новое описание." };
                    if(path === 'team.members') newItemTemplate = { img: "https://source.unsplash.com/300x300/?portrait", name: "Новый сотрудник", role: "Должность" };
                    if(path === 'events.items') newItemTemplate = { title: "Новое событие", desc: "Новое описание." };
                    if(path === 'materials.items') newItemTemplate = { title: "Новый материал", points: ["Пункт 1", "Пункт 2"] };
                    if(path === 'help.cards') newItemTemplate = { id: `new_card_${Date.now()}`, title: "Новая карточка", desc: "Описание", details: "", button: "" };

                    languages.forEach(lang => {
                        const { parent, key } = getParentAndKeyByPath(siteData[lang], path, true);
                        if (parent && Array.isArray(parent[key])) {
                            parent[key].push(deepCopy(newItemTemplate));
                        }
                    });
                    ...
                    confirmAction(message, () => {
                        languages.forEach(lang => {
                            const { parent, key } = getParentAndKeyByPath(siteData[lang], path);
                            if (parent && Array.isArray(parent[key]) && parent[key].length > index) {
                                parent[key].splice(index, 1);
                            }
                        });
                        isDirty = true;
                        renderAdminPanel(); // Перерисовываем только после успешного удаления
                    });
                    return; // Прерываем выполнение, чтобы не вызывать renderAdminPanel() до подтверждения
                }
            }
            isDirty = true;
            renderAdminPanel();
