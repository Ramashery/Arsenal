 ...
 * Находит самый длинный массив и дополняет остальные пустыми шаблонами.
 */
function synchronizeDataArrays() {
    ...
    pathsToSync.forEach(path => {
        ...
        // Находим максимальную длину массива среди всех языков
        languages.forEach(lang => {
            try {
                const arr = path.split('.').reduce((obj, key) => obj && obj[key], siteData[lang]);
                if (Array.isArray(arr)) {
                    ...
                    // Если по пути нет массива, создаем его
                    let current = siteData[lang];
                    const keys = path.split('.');
                    for(let i=0; i < keys.length - 1; i++) {
                        current = current[keys[i]];
                    }
                    current[keys[keys.length - 1]] = [];
                    arrays[lang] = current[keys[keys.length - 1]];
                }
            } catch (e) {
                console.warn(`Could not find or create path "${path}" for lang "${lang}" during sync.`);
            }
        });

        // Если есть что синхронизировать, дополняем короткие массивы
        if (Object.keys(arrays).length > 0) {
            languages.forEach(lang => {
                const arr = arrays[lang];
                if (arr && arr.length < maxLength) {
                    // Создаем шаблон для нового элемента
                    let newItemTemplate = {};
    ...
    if (wasModified) {
        isDirty = true; // Помечаем, что данные были изменены и требуют сохранения
...
function addAdminEventListeners() {
    ...
    document.querySelector('#admin-content-wrapper').addEventListener('click', (e) => {
        ...
        if (actionBtn) {
            ...
            // --- ИЗМЕНЕНИЕ: Упрощена функция подтверждения, убрана манипуляция с z-index ---
            ...
            if (path === 'treeCategories') { // Логика для глобальных категорий деревьев
                if (action === 'add') {
                ...
                // Синхронизированная логика для языковых массивов
                const languages = ['ru', 'en', 'ge'];

                const getTargetArray = (lang, pathString) => {
                    try {
                        return pathString.split('.').reduce((obj, key) => obj[key], siteData[lang]);
                    } catch (e) {
                        console.error(`Could not find path "${pathString}" for lang "${lang}"`);
                        return null;
                    }
                };
