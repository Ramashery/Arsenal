...
function addAdminEventListeners() {
    ...
    // Listener for the main content area (breadcrumbs, lang tabs, action buttons)
        ...
        if (actionBtn) {
            ...
            if (path === 'treeCategories') {
                if (action === 'add') {
                    ...
                    adminNotifications.show('Категория добавлена.', 'success');
                    ...
                    confirmAction(`Вы уверены, что хотите удалить категорию "${categoryName}"? Это действие необратимо.`, () => {
                        ...
                        adminNotifications.show('Категория удалена.', 'success');
                    });
                }
            } else {
                const languages = ['ru', 'en', 'ge'];

                const getTargetArrayByPath = (dataObject, pathString) => {
                    try {
                        return pathString.split('.').reduce((obj, key) => obj[key], dataObject);
                    } catch (e) {
                        console.error(`Не удалось найти путь "${pathString}" в объекте данных.`);
                        return null;
                    }
                };
                ...
                if (action === 'add') {
                    ...
                    languages.forEach(lang => {
                        const targetArray = getTargetArrayByPath(siteData[lang], path);
                        if (targetArray && Array.isArray(targetArray)) {
                            targetArray.push(deepCopy(newItemTemplate));
                        }
                    });
                    adminNotifications.show('Блок добавлен во всех языках.', 'success');
                    ...
                    confirmAction(message, () => {
                        ...
                        if (itemDeleted) {
                            adminNotifications.show('Блок удален из всех языков.', 'success');
                        }
                    });
                }
            }
