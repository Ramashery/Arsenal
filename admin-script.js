...
function getAdminSectionHTML(sectionKey) {
    ...
    switch (sectionKey) {
        case 'hero':
            ...
            html += renderLangTabs();
            html += createInput('Заголовок', `${currentAdminLang}.hero.title`, data.hero.title);
            html += createInput('Подзаголовок', `${currentAdminLang}.hero.subtitle`, data.hero.subtitle);
            html += createInput('Дата (текстовое описание)', `${currentAdminLang}.hero.date`, data.hero.date);
            html += createInput('Подписи к таймеру (Дней)', `${currentAdminLang}.countdownLabels.days`, data.countdownLabels.days);
            html += createInput('Подписи к таймеру (Часов)', `${currentAdminLang}.countdownLabels.hours`, data.countdownLabels.hours);
            html += `</div></div>`;
            break;
        case 'nav':
            html += createBreadcrumbs(sectionKey);
            ...
            Object.entries(data.nav).forEach(([key, value]) => {
                html += createInput(`Пункт "${key}"`, `${currentAdminLang}.nav.${key}`, value);
            });
            html += `</div></div>`;
            break;
        case 'about':
        ...
        case 'map':
            ...
            <div class="admin-card">
                ...
                <div class="admin-card-body">
                    ...
                    ${createInput('Заголовок карты', `${currentAdminLang}.map.title`, data.map.title)}
                    ${createTextarea('Описание карты', `${currentAdminLang}.map.desc`, data.map.desc)}
                    ${createTextarea('Текст о прозрачности', `${currentAdminLang}.map.transparency`, data.map.transparency)}
                </div>
            </div>`;
            break;
        case 'trees':
        ...
        case 'team':
            ...
            html += `<div class="admin-card"><div class="admin-card-body">${renderLangTabs()}${createInput('Заголовок секции', `${currentAdminLang}.team.title`, data.team.title)}</div></div>`;
        ...
        case 'contacts':
            html += createBreadcrumbs(sectionKey);
            html += renderLangTabs();
            html += `<div class="admin-card"><div class="admin-card-body">${createInput('Заголовок секции', `${currentAdminLang}.contacts.title`, data.contacts.title)}</div></div>`;
            html += `<div class="admin-card"><div class="admin-card-header"><h4>Контактная информация</h4></div><div class="admin-card-body">
                    ${createInput('Заголовок', `${currentAdminLang}.contacts.heading`, data.contacts.heading)}
                    ${createTextarea('Описание', `${currentAdminLang}.contacts.desc`, data.contacts.desc)}
                    ${createInput('Название локации', `${currentAdminLang}.contacts.locationName`, data.contacts.locationName)}
                    ${createInput('Адрес локации', `${currentAdminLang}.contacts.locationAddress`, data.contacts.locationAddress)}
                    ${createInput('Заголовок карты', `${currentAdminLang}.contacts.mapLocation`, data.contacts.mapLocation)}
                </div></div>`;
            html += `<div class="admin-card"><div class="admin-card-header"><h4>Форма обратной связи</h4></div><div class="admin-card-body">
                    ${createInput('Заголовок формы', `${currentAdminLang}.contacts.form.title`, data.contacts.form.title)}
                    ${createInput('Лейбл для имени', `${currentAdminLang}.contacts.form.name`, data.contacts.form.name)}
                    ${createInput('Лейбл для email', `${currentAdminLang}.contacts.form.email`, data.contacts.form.email)}
                    ${createInput('Лейбл для сообщения', `${currentAdminLang}.contacts.form.message`, data.contacts.form.message)}
                    ${createInput('Текст кнопки', `${currentAdminLang}.contacts.form.button`, data.contacts.form.button)}
                </div></div>`;
            break;
        case 'footer':
            html += createBreadcrumbs(sectionKey);
            html += `<div class="admin-card"><div class="admin-card-body">${renderLangTabs()}`;
            html += createTextarea('Девиз в футере', `${currentAdminLang}.footer.motto`, data.footer.motto);
            html += `</div></div>`;
            break;
        case 'global':
            html += createBreadcrumbs(sectionKey);
...
// ===== НАЧАЛО ИЗМЕНЕНИЙ: ИСПОЛЬЗУЕМ ГОТОВЫЕ ФУНКЦИИ FIREBASE =====
async function saveAdminChanges() {
    ...
    try {
        // Убрали динамический import, теперь используем готовые функции
        const { db, doc, setDoc } = window.firebaseApp;

        if (!db) {
            throw new Error("База данных Firebase не инициализирована. Проверьте HTML-файл.");
        }
