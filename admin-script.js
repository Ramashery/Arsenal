    ...
    ge: {
        ...
        about: {
            ...
            items: [
                ...
                { title: "ეკოლოგია", text: "ყოველი ხე — წვლილია ჰაერში, ნიადაგში, ქალაქის კლიმატში. ჩვენ არსენალის მთას ვუბრუნებთ სიმწვანეს, რომელიც ოდესღაც აქ იყო. ეს პატარა ნაბიჯია თბილისისთვის, მაგრამ პლანეტაზე ზრუნვის დიდი ნაწილის: ჩვენ ვირჩევთ სიცოცხლესა და ზრდას სიცარიელის ნაცვად." },
            ]
        },
        slider: [
...
function getAdminSectionHTML(sectionKey) {
    ...
    switch(sectionKey) {
        ...
        case 'global':
            ...
            html += `
            <div class="admin-form-group">
                <label for="global.mapIframeSrc">Код встраивания Google Карты (iframe)</label>
                <textarea id="global.mapIframeSrc" 
                          oninput="this.value = (this.value.match(/src="([^"]+)"/) || [null, this.value])[1];"
                          placeholder="Вставьте сюда код, скопированный из Google Карт...">${globalData.mapIframeSrc}</textarea>
                <div class="admin-form-help-text">
                    <strong>Как получить код:</strong><br>
                    1. Откройте нужное место на <a href="https://www.google.com/maps" target="_blank">Google Картах</a>.<br>
                    2. Нажмите кнопку "Поделиться".<br>
                    3. В открывшемся окне перейдите на вкладку "Встраивание карт".<br>
                    4. Нажмите "КОПИРОВАТЬ HTML".<br>
                    5. Вставьте скопированный код в это поле. Нужный URL будет извлечен автоматически.
                </div>
            </div>`;
            // --- END OF CHANGE ---
            html += `</div></div>`;
    ...
    setTimeout(() => {
        try {
            ...
            document.querySelectorAll('#admin-content-wrapper input, #admin-content-wrapper textarea').forEach(input => {
                ...
                let current;
                // Определяем, с какого объекта начинать сохранение.
                if (keys[0] === 'global' || currentAdminSection === 'trees') {
                    // Для глобальных настроек или раздела "Деревья" (у него сложная структура) начинаем с корня.
                    current = tempSiteData;
                } else {
                    // Для всех остальных языковых разделов начинаем с объекта текущего языка.
                ...
                if(keys.includes('points')) {
                   ...
                   let originalValue;
                    try {
                        // This logic is complex because paths can be relative or absolute.
                        // We check against the original 'siteData' object.
