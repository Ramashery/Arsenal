    ...
    setTimeout(() => {
        try {
            ...
            document.querySelectorAll('#admin-content-wrapper input, #admin-content-wrapper textarea').forEach(input => {
                ...
                // --- START OF FIX ---
                // Определяем, откуда начинать сохранение.
                // Если id поля начинается с 'global', 'ru', 'en', 'ge' (как в разделе "Деревья"), 
                // то путь абсолютный. В противном случае - путь относительный к текущему языку.
                let current;
                if (keys[0] === 'global' || keys[0] === 'ru' || keys[0] === 'en' || keys[0] === 'ge') {
                    current = tempSiteData; // Начинаем с корня объекта данных
                } else {
                    current = tempSiteData[currentAdminLang]; // Начинаем с объекта текущего языка
                }
                // --- END OF FIX ---
