...
function addAdminEventListeners() {
    ...
    // Обработка кликов в основной контент-зоне
    adminContentWrapper.addEventListener('click', (e) => {
        const breadcrumbLink = e.target.closest('.admin-breadcrumbs a[data-section]');
        if (breadcrumbLink) {
            e.preventDefault();
        ...
        const langTab = e.target.closest('.admin-lang-tabs button[data-lang]');
        if (langTab) {
        ...
        if (actionBtn) {
            ...
            const index = actionBtn.dataset.index ? parseInt(actionBtn.dataset.index, 10) : -1;

            // Функция подтверждения, которая решает проблему с z-index на мобильных
            ...
            // Особый случай: управление категориями деревьев (глобальные данные + языковые тексты)
            if (path === 'treeCategories') {
                if (action === 'add') {
                    const newId = `new_category_${Date.now()}`;
                    siteData.global.treeCategories.push({
                        id: newId,
                        quantity: 10,
                        costPerTree: 50,
                        sponsoredCount: 0,
                        img: 'https://source.unsplash.com/400x300/?tree,seedling'
                    });
                    // Добавляем текстовые заготовки для всех языков
                    ['ru', 'en', 'ge'].forEach(lang => {
                        siteData[lang].treeCategories[newId] = {
                            name: 'Новая категория',
                            desc: 'Описание для новой категории.'
                        };
                    });
                     adminNotifications.show('Категория дерева добавлена.', 'success');
                    ...
                    confirmAction(`Вы уверены, что хотите удалить категорию "${categoryName}"? Это действие необратимо.`, () => {
                        ...
                        // Удаляем текстовые данные из всех языков
                        ['ru', 'en', 'ge'].forEach(lang => {
                            delete siteData[lang].treeCategories[categoryId];
                        });
                        adminNotifications.show('Категория дерева удалена.', 'success');
                    });
                }
            }
            // Обработка всех остальных контентных блоков
            else {
                ...
                if (action === 'add') {
                    ...
                    if(path === 'slider') newItemTemplate = { img: "https://source.unsplash.com/1200x800/?nature,landscape", title: "Новый слайд", desc: "Новое описание." };
                    if(path === 'team.members') newItemTemplate = { img: "https://source.unsplash.com/300x300/?portrait,person", name: "Новый сотрудник", role: "Должность" };
                    ...
                    // Добавляем пустой блок во все языковые версии
                    languages.forEach(lang => {
                        // Безопасный доступ к вложенному массиву
                        const pathParts = path.split('.');
                        let targetArray = pathParts.reduce((obj, key) => obj && obj[key], siteData[lang]);
                        
                        if (Array.isArray(targetArray)) {
                            targetArray.push(deepCopy(newItemTemplate));
                        }
                    });
                    adminNotifications.show('Блок добавлен во все языковые версии.', 'success');
                    ...
                    confirmAction(message, () => {
                        ...
                        languages.forEach(lang => {
                            // Безопасный доступ к вложенному массиву для удаления
                            const pathParts = path.split('.');
                            let targetArray = pathParts.reduce((obj, key) => obj && obj[key], siteData[lang]);
                        ...
                        if(itemDeleted) {
                            adminNotifications.show('Блок удален из всех языковых версий.', 'success');
                        }
                    });
                }
            }
            ...
            isDirty = true;
            renderAdminPanel(); // Перерисовываем панель, чтобы отобразить изменения
        }
    });
}
