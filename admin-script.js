    ...
    setTimeout(() => {
        try {
            ...
            document.querySelectorAll('#admin-content-wrapper input, #admin-content-wrapper textarea').forEach(input => {
                ...
                // Определяем, является ли путь относительным (например, "team.members...")
                // или абсолютным (например, "ru.treeCategories...")
                if (keys[0] !== 'global' && keys[0] !== 'ru' && keys[0] !== 'en' && keys[0] !== 'ge') {
                    // Если путь относительный, добавляем текущий язык в начало пути
                    keys.unshift(currentAdminLang);
                }

                // Всегда начинаем с корня объекта данных
